...
`optimizer create [schematic name]` and it will create a config file in the `LabAPI\configs\DynamicSchematicOptimizer` path. Then you need to run `optimizer r` so the config will be applied to the server.
Also the plugin has culling so that the schematics won't be shown 24/7.
