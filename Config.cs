...
public class Config
{
    ...
    [Description(
        "Default config for schematics that are not optimized by MERO if the compatiblity is enabled, also will be the deafult for newly created schematics")]
    public SchematicOptimisationConfig DefaultConfig { get; set; } = new();

    public float CullingTickTimeInBetween { get; set; } = 1;

    [Description(
        "If true, DSO checks GitHub for a newer release on startup and downloads the plugin dll. The update is applied on the next server restart")]
    public bool EnableAutoUpdater { get; set; } = false;
